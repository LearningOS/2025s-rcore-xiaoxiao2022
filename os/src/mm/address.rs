...
// 在 SV39 模型中，虚拟地址是 39 位宽，但在 64 位系统中，地址需要扩展到 64 位。
// 如果虚拟地址的符号位（第 38 位）是 1，表示这是一个负地址。在扩展到 64 位时，符号扩展需要将高 25 位填充为 1，以保持地址的有符号语义。
// 虚拟地址空间划分:
// SV39 的虚拟地址空间分为两部分：
// 正地址空间: 符号位为 0，范围是 0x0000_0000_0000_0000 到 0x0000_007F_FFFF_FFFF。 低256G
// 负地址空间: 符号位为 1，范围是 0xFFFF_FFFF_FFFF_FFFF 到 0xFFFF_FF80_0000_0000。 高256G
// 负地址空间通常用于内核或特殊用途。
// 为什么不能直接返回 v.0 的低 39 位？
// 如果直接返回 v.0 的低 39 位，会丢失符号信息，导致地址的语义发生变化。例如：
// 一个负地址（符号位为 1）会被错误地解释为正地址。
// 这可能导致地址计算错误，尤其是在需要区分用户空间和内核空间的场景中。
